...
for j_scale = 1:numel(Scale) 
    ...
    [~,icidx] = min((uvs(1,:)-mean(uvs(1,:))).^2+(uvs(2,:)-mean(uvs(2,:))).^2);
    xcl = rl(:,icidx); origin = 0;
    ...
    idx = 1:numel(uvs(1,:)); idxl = idx(idx~=icidx);
    Sxl = transcoord(xcl,ncl,nxl,origin,rl(:,idxl));
    
    figure(2), subplot(2,2,j_scale),
    ...
    rhs = zeros(2*(order+2)*(order-1),1); rhs(1:4:end) = mul(idxl)'-mul(icidx);
    Fc = []; Fi = []; Fj = []; Fk = [];
    for k=n:-1:2
        for j=1:k
    ...
    Mmatrix = zeros(2*(order+2)*(order-1));
    ...
    err = NaN(size(xt1));
    err(idxlt) = diff;
    figure(3), subplot(2,2,j_scale),imagesc(log10(abs(err)))
